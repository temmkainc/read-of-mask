...
public class DeviceAdaptiveHint : MonoBehaviour
{
    ...
    [SerializeField] private string _xboxSpriteName;        // e.g. "xbox_x"
    [SerializeField] private string _playstationSpriteName; // e.g. "ps_square"
    ...
    public void Refresh()
    {
        ...
        {
            DeviceType.Xbox => $"<sprite=\"{_xboxSpriteAsset.name}\" name=\"{_xboxSpriteName}\"> {_label}",
            DeviceType.PlayStation => $"<sprite=\"{_playstationSpriteAsset.name}\" name=\"{_playstationSpriteName}\"> {_label}",
            _ => $"[<b>{_keyboardKey}</b>] {_label}"
        };
